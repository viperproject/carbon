...
class DefaultExpModule(val verifier: Verifier) extends ExpModule with DefinednessComponent with StatelessComponent {
  ...
  private def checkDefinednessComp(e: sil.Comp, error: PartialVerificationError, makeChecks: Boolean): Stmt = {
    val checks = components map (_.partialCheckDefinedness(e, error, makeChecks = makeChecks))
    val stmt = checks map (_._1())
    // check children similar as for quantified expressions, but avoid the body for now
    ...
    // check the body
    val permCheck = If(translateExp(e.filter.exp), checkDefinednessImpl(e.body, error, makeChecks), Nil)
    val stmt3 = handleQuantifiedLocals(e.variables, permCheck)
